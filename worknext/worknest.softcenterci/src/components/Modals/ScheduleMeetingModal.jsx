...
export default function ScheduleMeetingModal({ open, teamMembers, meeting, onClose, onCreate, onSave }) {
  ...
  function addGuestEmail() {
    ...
    if (!isValidEmail(email)) {
      setErrors((prev) => ({ ...prev, guestEmail: "Enter a valid email address." }));
      return;
    }
    if (guestEmails.includes(email)) {
      setErrors((prev) => ({ ...prev, guestEmail: "This email is already added." }));
      return;
    }
  ...
  function removeGuestEmail(email) {
    setGuestEmails((prev) => prev.filter((g) => g !== email));
  }
  ...
  function validate() {
    ...
    if (attendeeIds.length === 0 && guestEmails.length === 0) {
      next.attendees = "Add at least one attendee or guest.";
    }
  ...
  function handleSubmit(e) {
    ...
    const names = teamMembers.filter((m) => attendeeIds.includes(m.id)).map((m) => m.name);
    const payload = {
      title: title.trim(),
      date,
      time: time || "TBD",
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} noValidate className="space-y-3">
          <div>
            <label htmlFor="mt-title" className="text-xs font-medium text-slate-500 block mb-1">
              Title
            </label>
            <input
              id="mt-title"
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label htmlFor="mt-date" className="text-xs font-medium text-slate-500 block mb-1">
                Date
              </label>
              <input
                id="mt-date"
            ...
            <div>
              <label htmlFor="mt-time" className="text-xs font-medium text-slate-500 block mb-1">
                Time
              </label>
              <input
                id="mt-time"
          ...
          <div>
            <label htmlFor="mt-link" className="text-xs font-medium text-slate-500 block mb-1">
              Meeting Link
            </label>
            <input
              id="mt-link"
            ...
            />
            {errors.link && <p className="text-xs text-rose-500 mt-1">{errors.link}</p>}
          </div>

          <div>
            <span className="text-xs font-medium text-slate-500 block mb-2">Team Attendees</span>
          ...
          <div>
            ...
            {errors.guestEmail && <p className="text-xs text-rose-500 mt-1">{errors.guestEmail}</p>}
            ...
            {errors.attendees && <p className="text-xs text-rose-500 mt-1">{errors.attendees}</p>}
          </div>
